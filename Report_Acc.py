...
def test():
    ...
    if len(df_today_settlement) != 0: distinguish_settlement(df_today_settlement)
...
def distinguish_AC_no_data(df_today, df_yesterday):
    ...
    for x in range(len(today_tx_list)):
        if today_tx_list[x] in yester_tx_list:  # 如果這筆AC無資料 有存在於 昨天的 收單行無資料 且 訂單編號為 1對1對應
            ...
            df_today_dist_AC[df_today_dist_AC["交易序號"] == today_tx_list[x]].to_csv("Output.csv", mode="a",
        ...
        if yester_tx_list[x] in today_tx_list:  # 如果這筆收單行無資料 有存在於 昨天的 AC無資料 且 訂單編號為 1對1對應
            if (today_tx_list.count(yester_tx_list[x]) == 1 and today_tx_list[x] != "Null"):
                ...
                for x in list1: x.to_csv("Output.csv", mode="a", index=False, header=False)
            ...
            df_yesterday_Bank[df_yesterday_Bank["交易序號"] == yester_tx_list[x]].to_csv("Output.csv", mode="a",
                                                                                         index=False, header=False)
...
def distinguish_bank_no_data(df_today, df_yesterday):
    ...
    if len(df_today_dist_ctbc) != 0: df_today_dist_ctbc.to_csv("Output.csv", mode="a", index=False, header=False)
    df_today_dist_tspg_scrap = df_today_dist_tspg_scrap[
        df_today_dist_tspg_scrap["請款時間"] <= tspg_scrap_time]  # 爬帳昨晚九點以前的交易
    if len(df_today_dist_tspg_scrap) != 0: df_today_dist_tspg_scrap.to_csv("Output.csv", mode="a", index=False,
                                                                           header=False)
    df_today_dist_tspg_swp = df_today_dist_tspg_swp[
        df_today_dist_tspg_swp["請款時間"] <= tspg_swp_time]  # SWP 昨晚十點以前的交易
    if len(df_today_dist_tspg_swp) != 0: df_today_dist_tspg_swp.to_csv("Output.csv", mode="a", index=False,
                                                                       header=False)
    df_today_dist_ubot = df_today_dist_ubot[df_today_dist_ubot["請款時間"] <= ubot_time]  # 聯邦九點半以前的交易
    if len(df_today_dist_ubot) != 0: df_today_dist_ubot.to_csv("Output.csv", mode="a", index=False, header=False)
...
def distinguish_settlement(df_today_settlement):
    ...
    if len(df_settlement_abnormal) != 0: df_settlement_abnormal.to_csv("Output.csv", mode="a", index=False,
                                                                       header=False)
    df_settlement_exception = df_today_settlement[(df_today_settlement["收單行結帳日期"] != settle_time)]
    if len(df_settlement_exception) != 0: df_settlement_exception.to_csv("Output.csv", mode="a", index=False,
                                                                         header=False)
